package jp.co.internous.framepj.controller;

import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

import com.google.gson.Gson;

import jp.co.internous.framepj.model.form.UserForm;


/**
 * 認証に関する処理を行うコントローラー
 * @author インターノウス
 *
 */
@RestController
@RequestMapping("/frameweb/auth")
public class AuthController {
	
	/*
	 * フィールド定義
	 */
	
	
	private Gson gson = new Gson();
	
		
	/**
	 * ログイン処理をおこなう
	 * @param f ユーザーフォーム
	 * @return ログインしたユーザー情報(JSON形式)
	 */
	@PostMapping("/login")
	public String login(@RequestBody UserForm f) {
		//@Requestbodyとは「HTTPリクエストの中に入ってきたデータ(JSONなど)を、Javaオブジェクトに変換して受け取る」ためのアノテーション
		//jsからPOST通信で入力されたユーザー名とパスワードを変数fで受け取る
		//loginって言ってるから多分受け取った値をデータベース検索してresponse(レスポンス)を返さないといけない
	}
	
	/**
	 * ログアウト処理をおこなう
	 * @return 空文字
	 */
	@PostMapping("/logout")
	public String logout() {
		
	}

	/**
	 * パスワード再設定をおこなう
	 * @param f ユーザーフォーム
	 * @return 処理後のメッセージ
	 */
	@PostMapping("/resetPassword")
	public String resetPassword(@RequestBody UserForm f) {
		
	}
}
